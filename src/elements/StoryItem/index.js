...
const StoryItem = ({ story }) =>
  <>
    ...
    <Show
      when={( story.url )}
      fallback={(<a is='route-link' name='story' params={{storyId: story.id}}>{story.title}</a>)}
    >
    ...
    <div class='subtext light'>
      <Show
        when={( story.type !== 'job' )}
        fallback={( <a is='route-link' name='story' params={{storyId: story.id}}>{relativeTime(story.time * 1000)}</a> )}
      >
